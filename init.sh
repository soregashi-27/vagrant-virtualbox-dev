...
car << EOS | sudo tee -a /etc/apache2/envvars
...
mysqlId --initialize-insecure

mysql -u root << EOT
CREATE DATABASE shop;
CREATE USER phper@'%' IDENTIFIED WITH mysql_native_password BY 'secret';
GRANT ALL PRIVILEGES on shop.* to tomo@'%';
SECLET Host, User FROM mysql.user;

CREATE TABLE shop.items (
    id INT PRIMARY KEY AUTO_INCREMENT,
    nae VARCHAR(30) COMMENT '商品名',
    stock INT DEFAULT 0 COMMENT '在庫数'
) COMMENT='商品テーブル';

INSERT INTO shop.items (name, stock) VALUE("Perfect Protein", 12345);
EOT

echo "Hey! Installation completed."